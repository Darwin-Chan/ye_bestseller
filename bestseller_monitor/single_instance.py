...
界面进程与采集进程各持一把，用来回答「是不是已经有一个在跑」；数据交换台拿第三把
`EXCHANGE_LOCK`（窗口开着时锁在窗口进程手里，命令行与窗口同规——采集线票 14 的运行
互斥）；畅销品分析拿第四把 `ANALYSIS_LOCK`（窗口与 `--serve` 同规、共用一把——分析线
票 14 的运行互斥）。
...
# 数据交换台：同一台机器同一时刻至多一次交换台运行（窗口与命令行同规）。
EXCHANGE_LOCK = r"Local\bestseller_exchange"
# 畅销品分析：同一台机器同一时刻至多一次分析运行（窗口与 --serve 同规）。
