...
同一台机器同一时刻至多一次分析运行（窗口与 `--serve` 共用一把锁，锁的寿命＝分析的寿命：
匹配期间关窗要先确认「关闭并停止」，等收尾完成才关窗退出；抢不到锁的实例把已有窗口叫到
前面后退出 0）。

**走一遍**：选开始/结束日期 → 核对两侧「当日真实抓取」数量 → 进入同款确认 → 处理模型建议
（对比、移除、组内新增）→ 筛选与批量确认/撤回 → 暂时保存 → 查看畅销品 → 导出 HTML 报告。
进入同款确认后的逐对判断是长时的一段：页面盖匹配进度遮罩，实时报阶段与对数，可点「停止匹配」
停下（没判完的对记为可重试的失败、已判的留在判断缓存里），停后点「模型匹配同款」接着判、
命中缓存的不重复花钱。首次分析与重试共用这块遮罩。
...
**商品主图**：采集在详情提交链里一并保存商品信息版本与主图原始内容，资产按内容 SHA-256 去重，
存在库存库的 `product_image_assets` 表里（备份数据库即同时保留历史图片）。每次真实观测都重新
读主图，URL 不变也能识别内容变化；单图最多 8 MiB，一次失败自动重试一次、每次连接超时 5 秒，
最终失败原因记在信息版本里，不影响有效库存提交。分析取结束日及以前**最近一次**信息版本；
无历史图或当次失败时明确缺图，不借用旧图。开库只幂等建表，不回填旧图片。

**SKU 图**：详情页规格里配的颜色缩略图，一个 SKU 至多一张。与商品主图同规——与库存行同源
解析、不多开页面；每次真实观测重读重存；下载失败不挡库存提交、缺图不借用相邻观测。页面没配
图的 SKU（颜色随机型规格、单规格商品）以**当次观测的商品主图**代填，当次主图也缺就记无图行；
专属图 / 主图代填 / 无图三态在 `sku_image_versions.source` 里可区分。空图不是失败、不进周报
待办。每个 SKU 每次观测一行流水（含失败与无图行），字节同样进 `product_image_assets` 池；
量级上每观测每 SKU 一行、包内每周万行级。
...
独立重试某个失败的 SKU 图（流水行编号取自 `sku_image_versions.id`；周报待办只报失败条数、
不点行号，编号自己去这张表取）：

```bash
python -m bestseller_monitor.product_images --database <库存库路径> --retry-sku-image <行编号>
```

两个重试参数互斥、必给一个；重试的对象必须是失败行、且仍是该对象**最新的记录**（之后已有
更新的记录就当场拒绝），不改库存、不绕过同日去重；取得的内容按重试时间新增信息版本（SKU 图
按重试当下新开一行流水，成与不成如实记），保留原失败事实。主图重试没有重新访问详情页，
因此只记录图片证据，名称标记为待重新观测；下一次真实详情采集会形成新的完整版本。
...
| 商品主图与 SKU 图 | 都在库存库里：字节在 `product_image_assets` 内容寻址池（同图不重复存，备份库即保留历史图片）；观测流水在 `product_information_versions`（主图）与 `sku_image_versions`（SKU 图，每观测每 SKU 一行、含空图与失败，每周万行级） |
