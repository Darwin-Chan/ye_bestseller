...
测试也照这两件事实构造场景。采集壳与交换台壳的差异都收在 LauncherSpec 里，
这份用例对两种目标各钉一遍（与鼠标无关的部分）；两入口各自的参数守卫在
各自的文件里（test_exchange_launcher.py）。
...
class NotifyPolicyTests(unittest.TestCase):
    """弹窗政策按目标区分：采集壳「非零都弹」；交换台壳「0/1 静默，2 才弹」。"""
...
class ImportPurityTests(unittest.TestCase):
    ...
    def test_importing_a_launcher_does_not_load_project_modules(self):
        probe = (
            ...
            "loaded = [m for m in ('gui', 'exchange', 'bestseller_monitor') "
            "if m in sys.modules]; "
            "print(','.join(loaded))"
        )
        for module in ("gui_launcher", "exchange_launcher", "launcher_core"):
            done = subprocess.run(
