...
class TargetTests(unittest.TestCase):
    """两个目标各自的构建坐标：spec 是仓库里真有的文件，产物落 dist/，自检读 build/。"""

    def test_every_target_points_at_a_spec_that_exists(self):
        self.assertEqual(sorted(build_exe.TARGETS), ["exchange", "gui"])
        for key, target in build_exe.TARGETS.items():
            with self.subTest(target=key):
                ...
                self.assertEqual(target.toc.parts[-3:],
                                 ("build", f"bestseller_{key}", "Analysis-00.toc"))

    def test_the_exchange_target_builds_the_exchange_shell(self):
        self.assertEqual(build_exe.TARGETS["exchange"].spec.name, "bestseller_exchange.spec")
        self.assertEqual(build_exe.TARGETS["exchange"].exe.name, "bestseller_exchange.exe")
