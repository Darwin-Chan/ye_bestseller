...
| IS-19 | 2026-09-06 | **已改代码**：主路径 `crawl_store_by_click` 接入 `human.after_load()`（`read_delay_sec`）与 `human.before_action()`（`action_delay_sec`），使这两项拟人化延迟在 `pw_cdp` 下真正生效；仍在代码里的短等待已注明。 |
