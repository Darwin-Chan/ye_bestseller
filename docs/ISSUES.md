...
| IS-13 | 高 | 待处理 | **抗 deny 限流（指数退避 + 连击即停）** | 新一天全量（round #12）几乎全程被淘宝 deny 拉住：每家店约 33-35 分钟、只采到 33~40 个商品，且约**每 33s 一次 deny**（当前 `deny_backoff_sec=30` 单次退避挡不住）。**用户已确认方案。** | 明天实现：① `_capture_card` 遇 deny 指数退避（30→60→120s），**第三次仍 deny 直接跳过该商品**；② **10 分钟内 deny ≥6 次则停止本轮**（记日志+提示，避免无限被限流耗着）；退避/次数策略做成可配置。 |
