...
- 状态：已采用并实现（缺口被 [ADR-0030](0030-intervention-readings-from-one-evidence.md) 走完：本条把**证据**收成页面上的三样，但「解除判定」`resolved` 当时仍是另一份判据，只读地址与可见验证容器、看不见正文——于是正文命中的介入在 `observe()` 里不终止，正是本条「被否掉的方向」第 1 条描述过的那个机制，只是当时被归因到正在删的那条响应流信号上。0030 没有推翻本条的边界：三样证据、`SLIDER_MARKERS` / `LOGIN_MARKERS`、`<3000` 长度门一个都没动，206 份存档页上 `intervention_kind` 取值零变化）
- 日期：2026-09-19
